#!/bin/bash
# Generic single-model 3-stage pipeline runner.
# Usage: bash run_one.sh <ollama_model_tag> <alias>
set -e
cd "$(dirname "$0")"

MODEL="${1:?usage: bash run_one.sh <model> <alias>}"
ALIAS="${2:?usage: bash run_one.sh <model> <alias>}"
